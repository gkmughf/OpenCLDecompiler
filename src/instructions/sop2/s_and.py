        ...
        if self.suffix in {"b32", "b64"}:
            ...
            if is_reg(self.src1):
                ...
                if self.node.get_from_state(self.src1).type == RegisterType.WORK_DIM and self.src0.value == "0xffff":
                    ...
                    expr_node = self.get_expression_node(self.src1)
                elif self.node.get_from_state(self.src1).type == RegisterType.GLOBAL_SIZE_X and size_of_work_groups[0] == -int(
                    self.src0
                ):
                    new_value = make_op(
                ...
                elif self.node.get_from_state(self.src1).type == RegisterType.GLOBAL_SIZE_Y and size_of_work_groups[1] == -int(
                    self.src0
                ):
                    new_value = make_op(
                ...
                elif self.node.get_from_state(self.src1).type == RegisterType.GLOBAL_SIZE_Z and size_of_work_groups[2] == -int(
                    self.src0
                ):
                    new_value = make_op(
