...
from src.ir.registers.reg import get_reg_rang, expand_register_names, is_reg, Val
...
class VLshlrev(BaseInstruction):
    ...
    def to_fill_node(self):
        ...
        if self.suffix == "b64":
            ...
            start_from_register, end_from_register = (src_regs[0], src_regs[1]) if len(src_regs) == 2 else (src_regs[0], src_regs[0])

            new_value0 = make_op(self.node, start_from_register, Val(str(pow(2, int(self.src0.value)))), "*", suffix=self.suffix)
            new_value1 = make_op(self.node, end_from_register, Val(str(pow(2, int(self.src0.value)))), "*", suffix=self.suffix)
